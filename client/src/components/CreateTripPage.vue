...
const latitude = ref(13.7563); // default Bangkok
const longitude = ref(100.5018);
const province = ref(''); // readonly, filled by reverse geocode

// unified items array for images (type: 'new' only for create)
type Item = { id: string; type: 'new' | 'old'; url?: string; file?: File };
const items = ref<Item[]>([]); // max length 4
...
// --- Leaflet map + reverse geocode (Nominatim) ---
...
async function reverseGeocodeAndSetProvince(lat: number, lon: number) {
    try {
        ...
        // prefer state, then county, region, town, city
        ...
        <div class="max-w-3xl mx-auto px-6">
            ...
            <div class="card bg-[#DEDED1] dark:bg-base-100 p-10 rounded-2xl">
                ...
                <button @click="submitCreate" :disabled="isSubmitting"
                    class="btn bg-[#4A70A9] text-[#DEDED1] w-full mt-6 rounded-full">
                    <span v-if="isSubmitting" class="loading loading-spinner loading-sm mr-2"></span> Create Trip
                </button>
            </div>
        </div>
    </div>
